...
class SolutionOptimized:
    def setZeroes(self, matrix: List[List[int]]) -> None:
        ...
        Do not return anything, modify matrix in-place instead.
        """
        # the idea is to use the first row and first column to mark which rows and columns need to be set to zero; and we use a boolean variable to track if the first row needs to be set to zero (for the first column, since we don't modify the first tow, it will not affect the first column)
        rows, cols = len(matrix), len(matrix[0])
        rowZero = False
